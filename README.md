...
A desktop app for Windows, Mac, and Linux to play sound files with playlist support, VU meter, waveform visualization, and remote HTTP control.
...
- Remote HTTP control endpoints (/play, /pause, /stop, /next, /cue, /up, /down)
...
3. **File loading issues**: Verify file permissions and that the selected files are supported audio formats.
...
- **Memory usage**: Sidecar cache reduces repeated waveform rendering and metadata reads
...
2. Install dependencies:
   ```bash
   pip install pyqt5 numpy soundfile sounddevice
   ```
3. Run the app:
   ```bash
   python main.py
...
- Add automated tests for playback and cache flows
...
- `v0.4.2`: bugfixes, melhorias e integração HTTP com simulação real de teclas
...
```powershell
powershell -ExecutionPolicy Bypass -File .\installer\build_windows.ps1 -Version 0.4.2
```

Generated outputs:

- App bundle: `dist/LiveProPlayer/`
- Installer: `dist/installer/LiveProPlayer-setup-v0.4.2.exe`
...
```powershell
iscc /DMyAppVersion=0.4.2 .\installer\liveproplayer.iss
```
