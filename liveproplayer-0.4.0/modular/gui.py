...
class GuiMixin:
    ...
    def update_transport_button_state(self, state):
        ...
        if hasattr(self, 'cue_button'):
            ...
            cue_active = 'font-size: 24px; min-width: 60px; min-height: 60px; background-color: #ff9900; color: black; border: 2px solid #cc7a00; border-radius: 0px;'
            self.cue_button.setStyleSheet(cue_normal)
            def cue_press():
                self.cue_button.setStyleSheet(cue_active)
            def cue_release():
                self.cue_button.setStyleSheet(cue_normal)
