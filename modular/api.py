...
class RemoteControlRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if command == 'up':
            # Move seleção para cima na playlist
            if hasattr(self.server.player, 'on_move_selected_track_requested'):
                self.server.player.on_move_selected_track_requested(-1)
                self.send_response(200)
                self.send_header('Content-Type', 'application/json; charset=utf-8')
                self.end_headers()
        ...
        if command == 'down':
            # Move seleção para baixo na playlist
            if hasattr(self.server.player, 'on_move_selected_track_requested'):
                self.server.player.on_move_selected_track_requested(1)
                self.send_response(200)
                self.send_header('Content-Type', 'application/json; charset=utf-8')
