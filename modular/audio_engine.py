...
class AudioEngineMixin:
    ...
    def setup_vu_timer(self):
        ...
        self.remote_command_timer = QTimer()
        self.remote_command_timer.setInterval(50)
        self.remote_command_timer.timeout.connect(self.process_remote_commands)
        self.remote_command_timer.start()
    ...
    def play_audio(self, remove_silence_enabled=None):
        ...
        # Keep the cached entry in sync with the faded data that will actually play.
        # cache_audio_info may have stored the original (unfaded) samples; overwrite
        # the data field so update_preview_for_row and VU metering see the same audio.
        if hasattr(self, 'audio_cache') and file_path in self.audio_cache:
            self.audio_cache[file_path]['data'] = data
