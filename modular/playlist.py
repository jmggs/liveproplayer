        ...
        if hasattr(self, 'audio_cache') and file_path in self.audio_cache:
            ...
            if cached.get('data') is None:
                try:
                    ...
                    cached['duration_samples'] = len(data)
                    self.set_cached_duration(file_path, len(data) / samplerate)
                except Exception as e:
                    print(f"Error loading audio preview data: {e}")
            prev_vu_pos = getattr(self, 'vu_pos', 0)
            self.vu_data = cached.get('data')
            self.vu_samplerate = cached['samplerate']
            self.total_duration = cached['duration_samples']
            # Só zera vu_pos se não for um seek explícito
            if not hasattr(self, '_preserve_vu_pos') or not self._preserve_vu_pos:
                self.vu_pos = 0
            else:
